...
function combinations(nums, k) {
  let constructArray = [];
  let visited = new Set();
  function dfs(source, construct = []) {
    if (!visited.has(source) && construct.length < k) {
      visited.add(source);
      construct.push(source);
      if (construct.length === k) {
        constructArray.push([...construct]);
      } else {
        for (let num of nums) {
          if (!visited.has(num) && num > source) {
            dfs(num, construct);
          }
        }
      }
