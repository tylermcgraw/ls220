...
// Algorithm:
// need to find pairs of numbers where min(pair) * width
// 2 pointers: start/end. calculate volume, then if start < end increase start
...
function maxRainwater(heights) {
  ...
  while (start < end) {
    let volume = Math.min(heights[start], heights[end]) * (end - start);
    if (volume > maxVolume) {
      maxVolume = volume;
    }
